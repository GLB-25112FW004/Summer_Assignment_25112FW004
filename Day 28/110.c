...
struct BankAccount deposit(struct BankAccount acc){
    ...
    if(amount > 0){
        acc.balance += amount;     //Modifies the local copy
        printf("Success: Total updated balance is Rs%.2f\n", acc.balance);
    }else{
        printf("Error: Invalid deposit amount!\n");
    }
    return acc;       //Sends the modified copy to main
}
...
struct BankAccount withdraw(struct BankAccount acc){
    ...
    if(amount<=0){
        ...
        printf("Error: Insufficient balance! Your current balance is Rs%.2f\n", 
        acc.balance);
    }else{
        acc.balance -= amount;     //Modifies the local copy
        printf("Success: Please collect your cash. New balance is Rs%.2f\n",
        acc.balance);
    }
    return acc;    //Sends the modified copy back to main
}
